// src/components/SaveMovies.jsx
function SaveMovies({ favorites = [], toggleFavotites }) {
	const imgBase = "https://image.tmdb.org/t/p/w342";
	const fallback = "/no-poster.png"; // твоя заглушка

	if (!favorites.length) {
		return (
			<div className="hero-wrapper">
				<p style={{ opacity: 0.8 }}>Пока нет избранных фильмов</p>
			</div>
		);
	}
						...
						<div className="movies-card" key={item.id}>
							...
							<div className="movies-card__title">
								...
								<button
									...
									onClick={() => toggleFavotites(item)}
									// красим через currentColor, чтобы не спорить с CSS
									style={{
										color: isFav ? "#FF4D4F" : "#B3C0D2",
									}}
									aria-pressed={isFav}
									aria-label={
										isFav
